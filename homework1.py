...
# ---------- Q2 (19-dim) ----------
# [1.0 (bias), raw_length] + weekday one-hot (Mon..Sat, drop Sun -> 6) + month one-hot (Jan..Nov, drop Dec -> 11)

def featureQ2(datum, maxLen):
    raw_len = float(len(_get_text(datum)))
    _, month_num, weekday_num = _get_day_month_weekday(datum)

    w = np.zeros(6, dtype=float)      # 0..5 for Mon..Sat; Sunday dropped
    wi = int(weekday_num)
    if 0 <= wi <= 5:
        w[wi] = 1.0

    m = np.zeros(11, dtype=float)     # 1..11 for Jan..Nov; December dropped
    mi = int(month_num) - 1
    if 0 <= mi < 11:
        m[mi] = 1.0

    return np.concatenate([[1.0, raw_len], w, m]).astype(float)  # length 19

def Q2(dataset):
    maxLen = getMaxLen(dataset)  # kept for signature consistency
    X, Y = [], []
    for d in (dataset or []):
        r = _get_rating(d)
        if r is None: 
...
# ---------- Q3 (4-dim) ----------
# [1.0, raw_length, weekday_number, month_number]
...
def Q5(dataset, feat_func):
    ...
    # Use the same split + LR settings the grader expects
